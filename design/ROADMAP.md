...
| Web config UI | `main/web_ui.c` / `.html` | `esp_http_server` + embedded HTML. `GET /api/status`, `GET`/`POST /api/config`, `GET /api/log`, `POST /api/scan`, `POST /api/reboot`. Same NVS config as the console. SoftAP clients only; **no authentication yet**. Status panel shows the running build's version (`esp_app_desc_t`, i.e. `git describe --always --tags --dirty` at build time) so it can be compared against what the flasher page says it's about to flash. |
...
| Web flasher + CI | `docs/`, `.github/workflows/` | ESP Web Tools page, single US build. GitHub Actions builds `sdkconfig.defaults` unmodified and deploys to Pages; PRs build but don't deploy. `manifest.json` carries `version` (short commit hash), `commit` (full SHA) and `built_at` (UTC) - the flasher page fetches and displays them before the user clicks Connect & Flash, so what's about to be flashed is visible up front, not just inferable after the fact from the running firmware's own version string. |
