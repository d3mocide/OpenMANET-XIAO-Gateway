...
| Provisioning (NVS + console) | `main/provisioning.c` | Implemented: `gwcfg-show` / `gwcfg-set-uplink` / `gwcfg-set-softap` / `gwcfg-set-subnet` / `gwcfg-set-node` / `gwcfg-save` / `gwcfg-reset` over the serial console (now on the right USB peripheral), NVS blob load/save, shared validation, placeholder defaults. Not yet flashed/tested. |
...
- [ ] **Step 3a** - **If testing two gateways against one Pi, give them different local subnets**
      (`gwcfg-set-subnet`). Unicast works either way thanks to NAT, but ATAK embeds each device's
      private address in CoT payloads, so identical subnets can misdeliver direct messages rather
      than failing cleanly. Worth capturing a real CoT datagram during step 5 to confirm the
      `endpoint` attribute behaves as expected - see `TECHNICAL_REVIEW.md`.
